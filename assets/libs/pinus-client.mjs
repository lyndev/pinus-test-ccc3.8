...
Emitter.prototype.once = function(event, fn, target){
  ...
  function on() {
    self.off(event, on, target);
    fn.apply(target, arguments);
  }

  on.fn = fn;
  this.on(event, on, target);
  return this;
};
...
Emitter.prototype.removeEventListener = function(event, fn, target){
  ...
  for (var i = 0; i < callbacks.length; i++) {
    cb = callbacks[i];
    if ((cb.cb === fn || cb.cb.fn === fn) && (target === cb.self)) {
...
Emitter.prototype.emit = function(event){
  ...
  if (callbacks) {
    ...
    for (var i = 0, len = callbacks.length; i < len; ++i) {
      callbacks[i].cb.apply(callbacks[i].self, args);
    }
  }
...
(function() {
  ...
  var Package = Protocol.Package;
  var Message = Protocol.Message;
  var EventEmitter = window.EventEmitter // require("events");
  var rsa = window.rsa;
  ...
  var deCompose = function(msg) {
    ...
    if(protobuf && serverProtos[route]) {
      ...
      // lyndev modify 2024-2-20
      return unpack(new Uint8Array(msg.body))
    }
